            ...
            <Box w={variant === 'base' || variant === 'sm' ? '100dvw' : '80dvw'} h={HeightOfSlider[variant as keyof typeof HeightOfSlider]} display="flex" justifyContent="space-between" alignItems="center">
                ...
                <Box width="100vw" height="100vh" position="relative" overflow="hidden">
                    <Box
                        ...
                        boxShadow={`0 0 100px ${centerIndex === currentIndex ? 'white' : 'transparent'}`} 
                        width="15dvw"
                        borderRadius={'10px'}
                        overflow="hidden"
                    >
                    ...
                    <Box position="absolute" right={variant === 'base' || variant === 'sm' ? '2dvh' : '5dvh'} top="50%" transform="translateY(-50%)" width="10vw"  boxShadow={`0 0 50px ${rightIndex === rightIndex ? 'white' : 'transparent'}`}  > 
                        <img src={imagesSlider[rightIndex]} alt={`Slide ${rightIndex + 1}`} width="100%" height="100%" style={{ objectFit: 'cover' }} />
                    </Box>
                    <Box position="absolute" left={variant === 'base' || variant === 'sm' ? '2dvh' : '5dvh'} top="50%" transform="translateY(-50%)" width="10vw"  boxShadow={`0 0 50px ${leftIndex === leftIndex ? 'white' : 'transparent'}`} > 
